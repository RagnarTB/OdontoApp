...
public interface CitaRepository extends JpaRepository<Cita, Long> {
        ...
        @Query("SELECT new com.odontoapp.dto.ReporteDTO(c.estadoCita.nombre, COUNT(c)) " +
                        "FROM Cita c " +
                        "WHERE c.eliminado = false " +
                        "AND c.fechaHoraInicio BETWEEN :fechaInicio AND :fechaFin " +
                        "AND (:odontologoId IS NULL OR c.odontologo.id = :odontologoId) " +
                        "GROUP BY c.estadoCita.nombre")
